...
use crate::{FutureOption, FutureResult};
use std::future::Future;

#[async_trait]
impl<T> FutureOption<T> for Option<T> where T: Send + Sync + 'static {
    type Output<O: Send + Sync + 'static> = Option<O>;
    ...
    {
        if self.is_none() {
            let _ = std::mem::replace(self, Some(op().await));
        };
    ...
    {
       match (self, other) {
            (Some(a), Some(b)) => Some(op(a, b).await),
            _ => None,
        }
    }
    ...
    {
        matches!(self, Some(t) if op(t).await)
    }
...
#[async_trait]
impl<T, E> FutureResult<T, E> for Result<T, E> where T: Send + Sync + 'static, E: Send + Sync + 'static {
    type Output<A, B> = Result<A, B> where A: Send + Sync + 'static, B: Send + Sync + 'static;
    ...
    where
        ...
        Fut: Future<Output = bool> + Send + Sync + 'static
    {
        matches!(self, Ok(x) if op(x).await)
    }
    ...
    where
        ...
        Fut: Future<Output = bool> + Send + Sync + 'static
    {
        matches!(self, Err(x) if op(x).await)
    }
